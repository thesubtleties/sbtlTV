...
import { isBlockedUrl, checkProviderUrl } from './url-guard';

describe('url guard', () => {
  it('blocks loopback, private ranges, link-local and file urls', () => {
    for (const u of ['http://localhost/x', 'http://127.0.0.1:8080/', 'http://10.1.2.3/', 'http://172.16.0.1/', 'http://172.31.255.255/', 'http://192.168.1.1/', 'http://169.254.169.254/latest', 'file:///etc/passwd', 'http://[::1]/']) {
